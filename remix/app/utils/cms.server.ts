...
const cms = async <Data = unknown>(endpoint: string, populate?: string[] | null, filter?: string) => {
	const ctx = getPagesContext();
	const basename = ctx.env.CMS_URL ?? `http://localhost:1337`;
	const url = `${basename}/api${
		endpoint?.charAt(0) === "/"
			? endpoint
			: `/${endpoint}${!!filter ? filter : ""}${populate === null ? "" : `${!!filter ? '&' : '?'}populate=${populate?.join("&populate=") ?? "*"}`}`
	}`;
